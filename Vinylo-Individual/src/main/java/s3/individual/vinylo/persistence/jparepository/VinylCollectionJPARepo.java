...
public interface VinylCollectionJPARepo extends JpaRepository<VinylCollectionEntity, Integer> {
        ...
        // Deletes a vinyl record from the user's collection
        @Modifying
        @Transactional
        @Query("DELETE FROM VinylCollectionEntity vc WHERE vc.user.id = :userId AND vc.vinyl.id = :vinylId")
        void deleteByVinylIdAndUserId(
                        @Param("vinylId") int vinylId,
                        @Param("userId") int userId);
